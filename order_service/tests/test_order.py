...
from order_service.main import app, orders_db, order_items_db, current_order_id
...
def setup_function():
    """每个测试前重置数据库"""
    global orders_db, order_items_db, current_order_id
    orders_db.clear()
    order_items_db.clear()
    # 重置 current_order_id（注意：在 main.py 中它是模块级变量）
    import order_service.main
    order_service.main.current_order_id = 1
